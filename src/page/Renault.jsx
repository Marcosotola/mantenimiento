...
export const Renault = () => {
  ...
  const addOrEditPendiente = async (pendienteObject) => {
    try {
      if (currentId === "") {
        await db.collection("renault").doc().set(pendienteObject);
        ...
        await db.collection("renault").doc(currentId).update(pendienteObject);
  ...
  const onDeletePendiente = async (id) => {
    if (window.confirm("¿Desea eliminar el pendiente?")) {
      await db.collection("renault").doc(id).delete();
  ...
  useEffect(() => {
    const getPendientes = async () => {
      let query = db.collection("renault").orderBy("data", "asc");
      if (filterDate !== "") {
  ...
  return (
    <>
      <div>
        <h1 className="text-center text-primary m-5">Renault OT</h1>
